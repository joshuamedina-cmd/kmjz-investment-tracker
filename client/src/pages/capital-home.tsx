...
import {
  ...
  BALANCE_PLUS_LISTED_REPAYMENTS,
  RMLLC_FUNDED,
  RMLLC_ACCOUNTED,
  RMLLC_TO_JOSHUA,
  OLD_TEAM_TOTAL,
 ...
 return <div className="min-h-screen bg-slate-100 text-slate-950">
  ...
  <main className="mx-auto max-w-7xl px-5 py-7 space-y-7">
   <section className="grid gap-4 md:grid-cols-2">
    ...
    <Metric label="Current Reconciled Bank Balance" value={money(CASH_AFTER_JOSHUA_LOAN)} sub={`$66,751 reconciled balance less ${money(7_942.85)} now allocated to complete Joshua's loan`} onClick={()=>setReport("reconciliation")}/>
   </section>

   <section className="rounded-3xl bg-slate-950 p-6 text-white shadow-xl">
    <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><div className="text-xs font-bold uppercase tracking-[.18em] text-emerald-300">Capital Recovery View</div><h2 className="mt-2 text-2xl font-black">Total once the listed capital is repaid</h2><p className="mt-2 max-w-2xl text-sm text-slate-300">BTC $51,000 + DE.CON $6,250 + Lifted Industries $6,250 + Helicopter Shot $10,000 = {money(LISTED_REPAYMENTS_TOTAL)} expected back.</p></div><button onClick={()=>setReport("recoverable")} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-bold text-slate-950">View repayment detail <ChevronRight className="h-4 w-4"/></button></div>
    <div className="mt-6 grid gap-4 md:grid-cols-2"><div className="rounded-2xl bg-emerald-400 p-4 text-slate-950"><div className="text-xs font-bold uppercase tracking-wider">$66,751 balance + listed repayments</div><div className="mt-2 text-4xl font-black">{money(BALANCE_PLUS_LISTED_REPAYMENTS)}</div><div className="mt-1 text-xs font-medium">{money(LAST_RECONCILED_BALANCE)} reconciled balance + {money(LISTED_REPAYMENTS_TOTAL)} expected repayments.</div></div><div className="rounded-2xl bg-white/10 p-4"><div className="text-xs uppercase tracking-wider text-slate-300">Listed repayments</div><div className="mt-2 text-3xl font-black">{money(LISTED_REPAYMENTS_TOTAL)}</div><div className="mt-1 text-xs text-slate-400">Four specifically requested repayment items.</div></div></div>
    <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300"><span className="font-bold text-white">Adjusted cash after Joshua loan completion:</span> {money(CASH_AFTER_JOSHUA_LOAN)}. This is shown separately so the recovery total above still reflects the requested {money(LAST_RECONCILED_BALANCE)} + repayments view.</div>
   </section>
  ...
  {report&&<div className="fixed inset-0 z-50 bg-black/50 p-4 backdrop-blur-sm" onClick={close}><div className="mx-auto mt-4 max-h-[92vh] max-w-6xl overflow-y-auto rounded-3xl bg-slate-50 shadow-2xl" onClick={e=>e.stopPropagation()}><div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4"><div className="font-black">{report==="old-team"?"Old Team Pay Report":report==="rmllc"?"Rising Management LLC Report":report==="recoverable"?"Recoverable Capital / Repayment Report":report==="joshua-loan"?"Joshua $17,500 Loan Report":report==="fees"?"Fees Report":"Capital Reconciliation"}</div><button onClick={close} className="rounded-lg bg-slate-100 px-3 py-2 text-sm font-bold">Close</button></div><div className="p-6">
   {report==="old-team"&&<><div className="mb-5 grid gap-3 md:grid-cols-3"><Metric label="Total Old Team Pay" value={money(OLD_TEAM_TOTAL)}/><Metric label="Direct Main Account" value={money(4944)}/><Metric label="Through RMLLC" value={money(6000)}/></div><TxTable rows={oldTeamTransactions}/></>}
   ...
   {report==="joshua-loan"&&<><div className="mb-5 rounded-2xl bg-emerald-100 p-5"><div className="text-xs font-bold uppercase tracking-wider text-emerald-700">Loan fully allocated</div><div className="mt-1 text-3xl font-black">{money(17500)}</div><p className="mt-2 text-sm text-emerald-900">$2,500 cash draw + $6,557.15 RMLLC remainder + $8,442.85 taken from the main account. Remaining to fulfill: $0.</p></div><TxTable rows={joshuaLoanTransactions}/></>}
   {report==="recoverable"&&<><div className="mb-5 rounded-2xl bg-slate-950 p-5 text-white"><div className="text-sm text-slate-300">Specifically requested repayment pool</div><div className="mt-1 text-3xl font-black">{money(LISTED_REPAYMENTS_TOTAL)}</div><div className="mt-2 text-sm text-slate-300">Added to the $66,751 reconciled balance: <span className="font-bold text-white">{money(BALANCE_PLUS_LISTED_REPAYMENTS)}</span></div></div><div className="grid gap-3 md:grid-cols-2">{repaymentItems.map(x=><div key={x.entity} className="rounded-2xl border border-slate-200 bg-white p-4"><div className="text-sm font-bold">{x.entity}</div><div className="mt-1 text-2xl font-black">{money(x.amount)}</div><div className="mt-2 text-xs text-slate-500">{x.note}</div></div>)}</div><h3 className="mb-3 mt-6 font-black">Underlying recoverable positions</h3><div className="space-y-3">{recoverableItems.map(x=><div key={x.entity} className="rounded-xl border border-slate-200 bg-white p-4"><div className="flex justify-between gap-4"><div><div className="font-bold">{x.entity}</div><div className="mt-1 text-sm text-slate-500">{x.note}</div><div className="mt-2 text-xs font-semibold text-slate-400">{x.status}</div></div><div className="font-black">{money(x.amount)}</div></div></div>)}</div></>}
   ...
   {report==="reconciliation"&&<div className="space-y-4"><div className="rounded-2xl bg-white p-5"><div className="text-sm text-slate-500">Starting capital</div><div className="text-3xl font-black">{money(STARTING_CAPITAL)}</div></div><div className="rounded-2xl bg-white p-5"><div className="text-sm text-slate-500">Last reconciled bank balance</div><div className="text-3xl font-black">{money(LAST_RECONCILED_BALANCE)}</div></div><div className="rounded-2xl bg-white p-5"><div className="text-sm text-slate-500">Less Joshua loan completion taken now</div><div className="text-3xl font-black">{money(7_942.85)}</div></div><div className="rounded-2xl bg-emerald-100 p-5"><div className="text-sm text-emerald-700">Adjusted reconciled bank balance</div><div className="text-3xl font-black text-emerald-950">{money(CASH_AFTER_JOSHUA_LOAN)}</div></div></div>}
  </div></div></div>}
 </div>
}
