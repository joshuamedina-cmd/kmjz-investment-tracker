...
import { ChevronRight, DollarSign, FileText, Landmark, Receipt, RotateCcw, Users } from "lucide-react";
import {
  STARTING_CAPITAL,
 ...
 return <div className="min-h-screen bg-slate-100 text-slate-950">
  ...
  <main className="mx-auto max-w-7xl px-5 py-7 space-y-7">
   <section className="grid gap-4 md:grid-cols-3">
    ...
    <Metric label="Last Reconciled Bank Balance" value={money(LAST_RECONCILED_BALANCE)} sub="The ~$50K stated balance has been removed from this page." onClick={()=>setReport("reconciliation")}/>
    <Metric label="Cash After Completing Joshua Loan" value={money(CASH_AFTER_JOSHUA_LOAN)} sub="After treating the final loan completion amount as gone from the account now." onClick={()=>setReport("joshua-loan")}/>
   </section>
   ...
   <section className="rounded-3xl bg-slate-950 p-6 text-white shadow-xl">
    ...
    <div className="mt-6 grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-white/10 p-4"><div className="text-xs uppercase tracking-wider text-slate-300">Reconciled balance + repayments</div><div className="mt-2 text-3xl font-black">{money(BALANCE_PLUS_LISTED_REPAYMENTS)}</div><div className="mt-1 text-xs text-slate-400">Uses {money(LAST_RECONCILED_BALANCE)} bank balance.</div></div><div className="rounded-2xl bg-emerald-400 p-4 text-slate-950"><div className="text-xs font-bold uppercase tracking-wider">Projected after Joshua loan completion + repayments</div><div className="mt-2 text-3xl font-black">{money(PROJECTED_CASH_AFTER_REPAYMENTS)}</div><div className="mt-1 text-xs font-medium">Uses the adjusted cash balance after the final Joshua-loan amount is removed.</div></div><div className="rounded-2xl bg-white/10 p-4"><div className="text-xs uppercase tracking-wider text-slate-300">Listed repayments</div><div className="mt-2 text-3xl font-black">{money(LISTED_REPAYMENTS_TOTAL)}</div><div className="mt-1 text-xs text-slate-400">Four specifically requested repayment items.</div></div></div>
   </section>
  ...
  {report&&<div className="fixed inset-0 z-50 bg-black/50 p-4 backdrop-blur-sm" onClick={close}><div className="mx-auto mt-4 max-h-[92vh] max-w-6xl overflow-y-auto rounded-3xl bg-slate-50 shadow-2xl" onClick={e=>e.stopPropagation()}><div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4"><div className="font-black">{report==="old-team"?"Old Team Pay Report":report==="rmllc"?"Rising Management LLC Report":report==="recoverable"?"Recoverable Capital / Repayment Report":report==="joshua-loan"?"Joshua $17,500 Loan Report":report==="fees"?"Fees Report":"Capital Reconciliation"}</div><button onClick={close} className="rounded-lg bg-slate-100 px-3 py-2 text-sm font-bold">Close</button></div><div className="p-6">
   ...
   {report==="reconciliation"&&<div className="space-y-4"><div className="rounded-2xl bg-white p-5"><div className="text-sm text-slate-500">Starting capital</div><div className="text-3xl font-black">{money(STARTING_CAPITAL)}</div></div><div className="rounded-2xl bg-white p-5"><div className="text-sm text-slate-500">Last fully reconciled bank balance</div><div className="text-3xl font-black">{money(LAST_RECONCILED_BALANCE)}</div></div><div className="rounded-2xl bg-amber-50 p-5 text-amber-950"><div className="font-bold">Adjustment recorded now</div><div className="mt-1 text-2xl font-black">-{money(7942.85)}</div><div className="mt-1 text-sm">Completes Joshua's $17,500 loan and produces an adjusted cash position of {money(CASH_AFTER_JOSHUA_LOAN)}.</div></div></div>}
  </div></div></div>}
 </div>
}
